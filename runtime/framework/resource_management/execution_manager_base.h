...
class ExecutionManagerBase : public ExecutionManager {
 ...
 protected:
  ...
  // Clones the session state from the source session info to the target session
  // info.
  // - source: The source session info.
  // - target: The target session info.
  // Returns:
  // - OK if the session state is cloned successfully.
  absl::Status CloneSessionStateUnlocked(const SessionInfo& source,
                                         SessionInfo& target);

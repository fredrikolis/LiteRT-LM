...
absl::Status ExecutionManagerBase::CloneSessionStateUnlocked(
    const SessionInfo& source, SessionInfo& target) {
  ...
  if (source.sampler != nullptr) {
    ASSIGN_OR_RETURN(cloned_sampler,
      ...
      if (session_info->sampler != nullptr) {
        ...
        if (!decoded_ids_buffer_or.HasValue()) {
          FinishTaskAndLogErrors(
              task_id,
              absl::InternalError(
                  decoded_ids_buffer_or.Error().Message()),
              std::move(callback));
          return;
        }

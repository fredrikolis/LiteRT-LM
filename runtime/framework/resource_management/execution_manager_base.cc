...
absl::Status ExecutionManagerBase::ReleaseSessionUnlocked(SessionId session_id) {
  if (!session_lookup_.contains(session_id)) {
    return absl::InvalidArgumentError(
...
absl::Status ExecutionManagerBase::AddPrefillTask(
  ...
  auto task = [this, task_id, inputs = std::move(inputs)]() mutable -> void {
    ...
    absl::StatusOr<Responses> responses;
    {
      auto llm_executor_or = resource_manager_->AcquireExecutorWithContextHandler(
          session_info->context_handler);
      if (!llm_executor_or.ok()) {
        FinishTaskAndLogErrors(task_id, llm_executor_or.status(),
                               std::move(callback));
        return;
      }
      auto& llm_executor = *llm_executor_or.value();
      ...
      responses =
          Tasks::Prefill(llm_executor, *executor_inputs,
      ...
      if (cancelled != nullptr && cancelled->load()) {
        ...
        auto processed_tokens_or = llm_executor.GetProcessedTokens();
        if (!processed_tokens_or.ok()) {
          FinishTaskAndLogErrors(task_id, processed_tokens_or.status(),
                                 std::move(callback));
          return;
        }
        auto current_step_or = llm_executor.GetCurrentStep();
        if (!current_step_or.ok()) {
          FinishTaskAndLogErrors(task_id, current_step_or.status(),
                                 std::move(callback));
          return;
        }
        session_info->last_prefill_token_id =
            processed_tokens_or.value()
                ->GetTokenAtStep(current_step_or.value() - 1)
                .at(0);
      }
    }

    FinishTaskAndLogErrors(task_id, std::move(responses),
                           std::move(callback));
    return;
  };
...
absl::Status ExecutionManagerBase::AddDecodeTask(
  ...
  auto task = [this, task_id, constraint, cancelled,
    ...
    auto [session_info, cancelled_local, callback] = std::move(task_info.value());
    // If the session info is nullptr, it means the task is cancelled before it
    // is started.
    if (session_info == nullptr) {
      return;
    ...
    RETURN_IF_CANCELLED(cancelled_local, task_id, callback);
    ...
    absl::StatusOr<Responses> responses;
    {
      ...
      if (session_info->sampler != nullptr) {
        ...
        std::vector<int> decoded_ids(num_output_candidates,
                                      session_info->last_prefill_token_id);
        auto decoded_ids_buffer_nested_or =
            CopyToTensorBuffer<int>(decoded_ids, {num_output_candidates, 1});
        if (!decoded_ids_buffer_nested_or.HasValue()) {
          FinishTaskAndLogErrors(task_id,
                                 absl::InternalError(
                                     decoded_ids_buffer_nested_or.Error().Message()),
                                 std::move(callback));
          return;
        }
        decoded_ids_buffer = std::move(decoded_ids_buffer_nested_or.Value());
      }

      responses = Tasks::Decode(
          llm_executor, *tokenizer_, *session_info->stop_token_detector,
          num_output_candidates, session_info->benchmark_info, optional_sampler,
          constraint, std::move(decoded_ids_buffer), callback,
          cancelled_local.get(), max_output_tokens);
      if (!responses.ok() && absl::IsCancelled(responses.status())) {
        responses = Responses(TaskState::kCancelled);
      }

      if (cancelled_local != nullptr && cancelled_local->load()) {
        responses = Responses(TaskState::kCancelled);
      }
    }
...
absl::Status ExecutionManagerBase::AddCloneSessionTask(
    ...
    std::shared_ptr<std::atomic<bool>> absl_nonnull cancelled,
    absl::AnyInvocable<void(absl::StatusOr<Responses>)> callback) {
  // Logic for CloneSessionTask will be implemented in subclasses for now.
  return absl::UnimplementedError("CloneSessionTask not implemented in base.");
}
    ...
    absl::StatusOr<Responses> responses;
    {
      ...
      if (!decoded_ids_buffer_or.HasValue()) {
        FinishTaskAndLogErrors(
            task_id, absl::InternalError(decoded_ids_buffer_or.Error().Message()),
            std::move(callback));
        return;
      }
      ...
      responses = Tasks::Score(
          llm_executor, *tokenizer_, target_text, temperature,
          std::move(decoded_ids_buffer_or.Value()), store_token_lengths);

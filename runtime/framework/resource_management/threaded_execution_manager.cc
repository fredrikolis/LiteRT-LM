...
absl::Status ThreadedExecutionManager::WaitUntilDone(TaskId task_id,
  ...
  return session_and_task_lookup_mutex_.AwaitWithTimeout(absl::Condition(&task_done), timeout)
             ? absl::OkStatus()
...
absl::Status ThreadedExecutionManager::WaitUntilSessionDone(
  ...
  absl::MutexLock lock(&session_and_task_lookup_mutex_);
  return session_and_task_lookup_mutex_.AwaitWithTimeout(absl::Condition(&session_done), timeout)
             ? absl::OkStatus()
...
absl::Status ThreadedExecutionManager::FinishTask(
    ...
    absl::AnyInvocable<void(absl::StatusOr<Responses>)> callback) {
  {
    ...
    if (callback_thread_pool_ != nullptr) {
      ...
      callback(absl::InternalError("Callback thread pool is null, skipping "
